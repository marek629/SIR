...
void ConvertEffectsTest::addText_center() {
    ...
    conf.setTextPos(QPoint(50, 50));
    conf.setTextUnitPair(PosUnitPair(Percent, Percent));
    QFont font = QFont("DejaVu Sans");
    font.setPointSize(20);
    conf.setTextFont(font);
    conf.setTextString("test string");
    conf.setTextPosModifier(Center);
    conf.setTextColor(Qt::green);
    conf.setTextOpacity(0.5);
    conf.setTextRotation(0);
    conf.setTextFrame(false);
    info.setEffectsConfiguration(conf);

    effects.addText();

    conf = info.effectsConfiguration();
    QImage expected(testImg);
    QPoint centerPoint(expected.width()/2, expected.height()/2);
    QFontMetrics fontMetrics(conf.getTextFont(), &expected);
    QRect rect = fontMetrics.boundingRect(conf.getTextString());
    const int dx = 5;
    const int dy = 1;
    rect.adjust(-dx, -dy, dx, dy);
    rect.moveCenter(centerPoint);
    QPainter p(&expected);
    p.setPen(conf.getTextColor());
    p.setFont(conf.getTextFont());
    p.setOpacity(conf.getTextOpacity());
    p.drawText(rect, Qt::AlignCenter, conf.getTextString());
...
void ConvertEffectsTest::addText_topLeftCorner() {
    ...
    EffectsConfiguration conf;
    conf.setTextPos(QPoint(0, 0));
    conf.setTextUnitPair(PosUnitPair(Pixel, Pixel));
    QFont font = QFont("DejaVu Sans");
    font.setPointSize(20);
    conf.setTextFont(font);
    conf.setTextString("test string");
    conf.setTextPosModifier(TopLeftCorner);
    conf.setTextColor(Qt::green);
    conf.setTextOpacity(0.5);
    conf.setTextRotation(0);
    conf.setTextFrame(false);
...
void ConvertEffectsTest::addText_middleBottomEdge() {
    ...
    EffectsConfiguration conf;
    conf.setTextPos(QPoint(50, 100));
    conf.setTextUnitPair(PosUnitPair(Percent, Percent));
    QFont font = QFont("DejaVu Sans");
    font.setPointSize(20);
    conf.setTextFont(font);
    conf.setTextString("test string");
    conf.setTextPosModifier(MiddleBottomEdge);
    conf.setTextColor(Qt::green);
    conf.setTextOpacity(0.5);
    conf.setTextRotation(0);
    conf.setTextFrame(false);
